...
export class MysqlService implements OnModuleDestroy {
    ...
    async beginTransaction(): Promise<void> {
        try {
          await this.pool.getConnection().then(conn => conn.beginTransaction());
        } catch (error) {
      ...
      async commit(): Promise<void> {
        try {
          await this.pool.getConnection().then(conn => conn.commit());
        } catch (error) {
      ...
      async rollback(): Promise<void> {
        try {
          await this.pool.getConnection().then(conn => conn.rollback());
        } catch (error) {
